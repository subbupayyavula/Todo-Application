...
const sumbmitHandle=(e)=>{
 e.preventDefault();
 axios.put(`http://localhost:4000/todo1/${id}`,data)
 .then(()=>{
  alert("todo updated successfull");
   navigate('/Navbar')
 })
 .catch(err=>{
   console.log( 'failed to post the data',err)
...
<div  className=' h-[272px] w-[400px] rounded-[10px] relative top-[150px] ml-[500px] shadow-xl border-2 border-solid border-blue-600  '>
      ...
      <button className='bg-btn mt-3 ml-[50px] rounded-[10px] w-[301px] h-[30px] font-semibold font-inder' type='submit'>Edit Todo</button> 
      </form>
      </div>
</div>
  ) 
